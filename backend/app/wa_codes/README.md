...
- **Deleting a code** — `DELETE /wa-codes/{id}` is guarded: `GET /wa-codes/{id}/connections` returns reference counts across all six referencing tables (`work_auth_project_codes`, `work_auth_building_codes`, `rfa_project_codes`, `rfa_building_codes`, `deliverable_wa_code_triggers`, `sample_type_wa_codes`). The DELETE returns 409 if any count is nonzero. This is an application-level guard; the DB also enforces `ondelete="RESTRICT"` on all but `deliverable_wa_code_triggers` (which is CASCADE — the API guard is what makes that safe to delete through).
