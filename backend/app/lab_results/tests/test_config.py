...
class TestListSampleTypes:
    async def test_returns_200(
        ...
        await seed_sample_type(db_session, name="Bulk ACM")
    ...
    async def test_includes_created_type(
        ...
        await seed_sample_type(db_session, name="Mold Air")
        response = await auth_client.get(BASE)
...
class TestCreateSampleType:
    ...
    async def test_duplicate_name_returns_409(
        ...
        await seed_sample_type(db_session, name="Duplicate Type")
...
class TestGetSampleType:
    async def test_returns_200(
        ...
        st = await seed_sample_type(db_session, name="Get Test Type")
...
class TestUpdateSampleType:
    async def test_patch_name(self, auth_client: AsyncClient, db_session: AsyncSession):
        st = await seed_sample_type(db_session, name="Old Name")
    ...
    async def test_patch_allows_multiple_inspectors(
        ...
        st = await seed_sample_type(
            db_session, name="Multi Inspector Type", allows_multiple_inspectors=True
        )
...
class TestDeleteSampleType:
    async def test_returns_204(
        ...
        st = await seed_sample_type(db_session, name="To Delete Type")
...
class TestSubtypes:
    async def test_add_subtype_returns_201(
        ...
        st = await seed_sample_type(db_session, name="Subtype Parent")
        response = await auth_client.post(
    ...
    async def test_subtype_appears_on_parent(
        ...
        st = await seed_sample_type(db_session, name="Subtype List Parent")
    ...
    async def test_delete_subtype_returns_204(
        ...
        st = await seed_sample_type(db_session, name="Subtype Delete Parent")
    ...
    async def test_delete_subtype_wrong_parent_returns_404(
        ...
        st1 = await seed_sample_type(db_session, name="Parent A Subtypes")
        st2 = await seed_sample_type(db_session, name="Parent B Subtypes")
        add = await auth_client.post(
...
class TestUnitTypes:
    async def test_add_unit_type_returns_201(
        ...
        st = await seed_sample_type(db_session, name="Unit Type Parent")
        response = await auth_client.post(
    ...
    async def test_unit_type_appears_on_parent(
        ...
        st = await seed_sample_type(db_session, name="Unit Type List Parent")
        await auth_client.post(
            f"{BASE}/{st.id}/unit-types", json={"name": "TEM Sample"}
        )
        response = await auth_client.get(f"{BASE}/{st.id}")
        unit_types = response.json()["unit_types"]
        assert any(u["name"] == "TEM Sample" for u in unit_types)

    async def test_add_unit_type_missing_type_returns_404(
    ...
    async def test_delete_unit_type_returns_204(
        ...
        st = await seed_sample_type(db_session, name="Unit Type Delete Parent")
        add = await auth_client.post(
    ...
    async def test_delete_unit_type_wrong_parent_returns_404(
        ...
        st1 = await seed_sample_type(db_session, name="Parent A Unit Types")
        st2 = await seed_sample_type(db_session, name="Parent B Unit Types")
        add = await auth_client.post(
        ...
        response = await auth_client.delete(
            f"{BASE}/{st2.id}/unit-types/{unit_type_id}"
        )
        assert response.status_code == 404
...
class TestTurnaroundOptions:
    async def test_add_turnaround_option_returns_201(
        ...
        st = await seed_sample_type(db_session, name="TAT Parent")
        response = await auth_client.post(
    ...
    async def test_turnaround_appears_on_parent(
        ...
        st = await seed_sample_type(db_session, name="TAT List Parent")
        await auth_client.post(
            f"{BASE}/{st.id}/turnaround-options",
    ...
    async def test_delete_turnaround_option_returns_204(
        ...
        st = await seed_sample_type(db_session, name="TAT Delete Parent")
        add = await auth_client.post(
            f"{BASE}/{st.id}/turnaround-options",
    ...
    async def test_delete_tat_wrong_parent_returns_404(
        ...
        st1 = await seed_sample_type(db_session, name="Parent A TAT")
        st2 = await seed_sample_type(db_session, name="Parent B TAT")
        add = await auth_client.post(
...
class TestRequiredRoles:
    async def test_add_required_role_returns_201(
        ...
        st = await seed_sample_type(db_session, name="Role Parent")
        response = await auth_client.post(
    ...
    async def test_role_appears_on_parent(
        ...
        st = await seed_sample_type(db_session, name="Role List Parent")
        await auth_client.post(
            f"{BASE}/{st.id}/required-roles",
    ...
    async def test_duplicate_role_returns_409(
        ...
        st = await seed_sample_type(db_session, name="Role Dupe Parent")
        await auth_client.post(
            f"{BASE}/{st.id}/required-roles",
            json={"role_type": EmployeeRoleType.ACM_AIR_TECH},
        )
    ...
    async def test_different_roles_same_type_ok(
        ...
        st = await seed_sample_type(db_session, name="Role Multi Parent")
    ...
    async def test_delete_required_role_returns_204(
        ...
        st = await seed_sample_type(db_session, name="Role Delete Parent")
        add = await auth_client.post(
            f"{BASE}/{st.id}/required-roles",
    ...
    async def test_delete_role_wrong_parent_returns_404(
        ...
        st1 = await seed_sample_type(db_session, name="Parent A Roles")
        st2 = await seed_sample_type(db_session, name="Parent B Roles")
        add = await auth_client.post(
...
class TestWACodes:
    async def test_add_wa_code_returns_201(
        ...
        st = await seed_sample_type(db_session, name="WA Code Parent")
        wc = await seed_wa_code(db_session, code="ACM-WA-001")
        response = await auth_client.post(
    ...
    async def test_wa_code_appears_on_parent(
        ...
        st = await seed_sample_type(db_session, name="WA Code List Parent")
        wc = await seed_wa_code(db_session, code="ACM-WA-002")
    ...
    async def test_duplicate_wa_code_returns_409(
        ...
        st = await seed_sample_type(db_session, name="WA Code Dupe Parent")
        wc = await seed_wa_code(db_session, code="ACM-WA-003")
        await auth_client.post(f"{BASE}/{st.id}/wa-codes", json={"wa_code_id": wc.id})
        response = await auth_client.post(
            f"{BASE}/{st.id}/wa-codes", json={"wa_code_id": wc.id}
        )
    ...
    async def test_missing_wa_code_returns_404(
        ...
        st = await seed_sample_type(db_session, name="WA Code 404 Parent")
        response = await auth_client.post(
    ...
    async def test_add_wa_code_missing_type_returns_404(
        ...
        wc = await seed_wa_code(db_session, code="ACM-WA-005")
        response = await auth_client.post(
    ...
    async def test_delete_wa_code_returns_204(
        ...
        st = await seed_sample_type(db_session, name="WA Code Delete Parent")
        wc = await seed_wa_code(db_session, code="ACM-WA-006")
        await auth_client.post(f"{BASE}/{st.id}/wa-codes", json={"wa_code_id": wc.id})
    ...
    async def test_delete_wa_code_not_linked_returns_404(
        ...
        st = await seed_sample_type(db_session, name="WA Code Unlinked Parent")
        wc = await seed_wa_code(db_session, code="ACM-WA-007")
        response = await auth_client.delete(f"{BASE}/{st.id}/wa-codes/{wc.id}")
        assert response.status_code == 404
